...
export class Animal {
  ...
  constructor(props: AnimalProps) {
    ...
    this.fechaNacimiento = props.fechaNacimiento instanceof Date 
      ? new Date(
  ...
  public getEdad(): { años: number; meses: number; dias: number; texto: string } {
    const hoy = new Date();
  ...
  public toJSON() {
    // Para enviar a Supabase - SOLO la fecha en formato YYYY-MM-DD
    ...
    return {
      ...
      fecha_nacimiento: `${año}-${mes}-${dia}`, // Solo fecha, sin hora
      sexo: this.sexo,
  ...
  static fromSupabase(data: any): Animal {
